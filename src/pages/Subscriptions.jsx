...
const getRenewalUrgency = (nextRenewal) => {
    ...
    if (days <= 2) {
        return {
            ...
            className: 'sub-urgency-critical',
        }
    }
    return {
        label: `Due in ${days} day${days === 1 ? '' : 's'}`,
        className: 'sub-urgency-warning',
    }
}
    ...
    const formContent = (
        <>
            <div className="mb-3.5">
                <p className="text-[13px] font-semibold text-[var(--color-text-primary)]">Subscription details</p>
                <p className="mt-1 text-[11px] text-[var(--color-text-secondary)]">Add your monthly cost and renewal day to keep this list up to date.</p>
            </div>
            ...
            <form onSubmit={handleSave} className="space-y-3">
                <div>
                    <label className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-secondary)]">Name</label>
                    <input
                        value={form.name}
                        onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))}
                        placeholder="Spotify, Notion, Netflix"
                        className="mt-1.5 w-full rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] px-3 py-2 text-[13px] text-[var(--color-text-primary)] outline-none transition focus:bg-[var(--color-surface)]"
                    />
                </div>

                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                        <label className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-secondary)]">Amount</label>
                        <input
                            value={form.amount}
                            ...
                            step="0.01"
                            placeholder="0.00"
                            className="mt-1.5 w-full rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] px-3 py-2 text-[13px] text-[var(--color-text-primary)] outline-none  focus:bg-[var(--color-surface)]"
                        />
                    </div>
                    <div>
                        <label className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-secondary)]">Currency</label>
                        <select
                            value={form.currency}
                ...
                <div>
                    <label className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-secondary)]">Renewal date</label>
                    <input
                        ...
                        className="mt-1.5 w-full rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] px-3 py-2 text-[13px] text-[var(--color-text-primary)] outline-none transition focus:bg-[var(--color-surface)]"
                    />
                    <p className="mt-1.5 text-[10px] text-[var(--color-text-secondary)]">Repeats monthly on the same day.</p>
                </div>

                <div className="rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-surface-soft)] px-3 py-2 text-[12px] text-[var(--color-text-primary)]">
                    <div className="flex items-center justify-between gap-2">
                        <p className="font-semibold">LKR conversion</p>
                        <span className="rounded-full bg-[var(--color-surface)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-text-secondary)]">Live</span>
                    </div>
                    <p className="mt-1.5 text-[12px] text-[var(--color-text-secondary)]">{conversionSummary}</p>
                </div>
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <button
                        type="submit"
                        disabled={saving}
                        className="inline-flex items-center justify-center rounded-lg bg-[var(--mx-color-c6ff00)] px-4 py-2 text-[12px] font-semibold text-black transition hover:bg-[var(--mx-color-a8db00)] disabled:opacity-60"
                    >
                    ...
                    {editing && (
                        <button
                            ...
                            className="inline-flex items-center justify-center rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-4 py-2 text-[12px] font-semibold text-[var(--color-text-primary)]"
                        >
                            Cancel edit
                        </button>
                    )}
        ...
        ? createPortal(
            <div className="fixed inset-0 z-[130] bg-black/35 backdrop-blur-sm p-2 sm:p-4 lg:hidden" onClick={closeMobileOverlay}>
                <div className="flex min-h-full items-center justify-center">
                    <div
                        className="lifesync-soft-borders w-full max-w-[440px] max-h-[calc(100dvh-1rem)] overflow-y-auto rounded-2xl border border-[var(--color-border-soft)] bg-[var(--color-surface)] p-3.5 shadow-[0_18px_48px_rgba(17,24,39,0.2)] sm:max-h-[calc(100dvh-2rem)]"
                        onClick={(event) => event.stopPropagation()}
                    >
                        <div className="mb-2.5 flex items-center justify-between">
                            <p className="text-[13px] font-semibold text-[var(--color-text-primary)]">{editing ? "Edit subscription" : "Add subscription"}</p>
                            <button
                                type="button"
                                onClick={closeMobileOverlay}
                                className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-surface-muted)] text-[var(--color-text-primary)]"
                                aria-label="Close"
                            >
            ...
            <div className="mx-auto max-w-[1160px] space-y-4 px-0.5 sm:space-y-5">
                <div className="rounded-2xl border border-[var(--color-border-soft)] bg-[var(--color-surface)] px-4 py-3.5 shadow-sm sm:px-5 sm:py-4.5">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                        ...
                        <button
                            ...
                            className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface-muted)] px-3.5 py-2 text-[12px] font-semibold text-[var(--color-text-primary)] transition hover:bg-[var(--color-surface)]"
                        >
                ...
                <div className="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)] lg:gap-5">
                    <div className="hidden lg:block">
                        <div className="sticky top-5 rounded-2xl border border-[var(--color-border-soft)] bg-[var(--color-surface)] p-4 shadow-sm">
                            {formContent}
                        </div>
                    </div>

                    <div className="rounded-2xl border border-[var(--color-border-soft)] bg-[var(--color-surface)] p-3 shadow-sm sm:p-4">
                        <div className="mb-3 grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto] sm:items-center">
                            <div className="rounded-xl border border-[var(--color-border-soft)] bg-[var(--color-surface-soft)] px-3 py-2.5">
                                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--color-text-secondary)]">Monthly total</p>
                                <p className="mt-0.5 text-[19px] font-bold text-[var(--color-text-primary)]">{formatCurrency(subscriptionSummary.totalLkr, "LKR")}</p>
                                {subscriptionSummary.unavailableCount > 0 && (
                                    <p className="mt-1 text-[10px] text-[var(--color-text-secondary)]">{subscriptionSummary.unavailableCount} conversion unavailable</p>
                                )}
                            </div>
                            <div className="flex items-center gap-2 rounded-xl border border-[var(--color-border-soft)] bg-[var(--color-surface-soft)] px-3 py-2">
                                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-surface-muted)] text-[var(--color-text-primary)]">
                                    <FaWallet className="h-3.5 w-3.5" />
                                </div>
                                <div>
                                    <p className="text-[12px] font-semibold text-[var(--color-text-primary)]">Your subscriptions</p>
                                    <p className="text-[10px] text-[var(--color-text-secondary)]">Sorted by nearest renewal</p>
                                </div>
                            </div>
                        </div>
                        ...
                        {loading ? (
                            <div className="mt-4 flex items-center gap-2 text-[12px] text-[var(--color-text-secondary)]">
                                <div className="h-3 w-3 animate-spin rounded-full border-2 border-[var(--color-text-primary)]/60 border-t-transparent"></div>
                                Loading subscriptions...
                            </div>
                        ) : subscriptions.length === 0 ? (
                            <div className="mt-2 rounded-xl border border-dashed border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] px-4 py-7 text-center text-[12px] text-[var(--color-text-secondary)]">
                            ...
                            <div className="mt-2 max-h-[560px] space-y-2 overflow-y-auto pr-1.5 sm:space-y-2.5">
                                {getSortedSubscriptions(subscriptions).map((item) => {
                                    ...
                                    return (
                                        <div key={item.id} className="rounded-xl border border-[var(--color-border-soft)] bg-[var(--color-surface-soft)] p-3">
                                            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                                                <div className="min-w-0">
                                                    <div className="flex flex-wrap items-center gap-1.5">
                                                        <p className="truncate text-[13px] font-semibold text-[var(--color-text-primary)]">{item.name}</p>
                                                        <span className="rounded-full bg-[var(--color-surface-muted)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-text-secondary)]">{item.currency}</span>
                                                        <span className="rounded-full bg-[var(--color-surface-muted)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-text-secondary)]">{item.currency === "LKR" ? "Local" : "Converted"}</span>
                                                    </div>
                                                ...
                                                {urgency ? (
                                                    ...
                                                    <span className="inline-flex rounded-full border border-[var(--color-border-soft)] bg-[var(--color-surface)] px-2 py-0.5 text-[10px] font-semibold text-[var(--color-text-secondary)]">
                                                        On track
                                                    </span>
                                                )}
                                            </div>
                                            ...
                                            <div className="mt-2.5 flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
                                                <div className="rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-surface)] px-2.5 py-1.5 text-[12px] text-[var(--color-text-secondary)]">
                                                    {item.currency === "LKR"
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    <button
                                                        ...
                                                        className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-2.5 py-1.5 text-[11px] font-semibold text-[var(--color-text-primary)]"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="sub-delete-btn inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold transition"
                                                    >
