    ...
    return (
        <div className="space-y-8 sm:space-y-10 lg:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            <header className="space-y-2 px-1">
                <h2 className="text-[28px] sm:text-[32px] lg:text-[36px] font-bold text-[#1d1d1f] tracking-tight leading-tight">
                    Good morning, <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#1d1d1f] to-[#86868b]">{firstName}</span>.
                </h2>
                <p className="text-[#86868b] text-[15px] sm:text-[17px] lg:text-[19px] font-medium">Here is what is happening with your account today.</p>
            </header>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                {[
                    ...
                    <div key={i} className="bg-white rounded-[24px] sm:rounded-[28px] lg:rounded-[32px] p-5 sm:p-6 lg:p-8 shadow-sm border border-[#d2d2d7]/50 hover:shadow-xl hover:shadow-black/[0.02] transition-all duration-500 group active:scale-95 sm:active:scale-100">
                        <div className={`w-10 sm:w-11 lg:w-12 h-10 sm:h-11 lg:h-12 rounded-xl sm:rounded-[16px] lg:rounded-2xl flex items-center justify-center mb-4 sm:mb-5 lg:mb-6 shadow-lg shadow-black/[0.05] transition-transform duration-500 group-hover:scale-110`} style={{ backgroundColor: `${stat.color}15` }}>
                            <svg className="w-5 sm:w-5 lg:w-6 h-5 sm:h-5 lg:h-6" fill="none" stroke={stat.color} viewBox="0 0 24 24" strokeWidth={2}>
                                {stat.icon}
                            </svg>
                        </div>
                        <p className="text-[#86868b] text-[11px] sm:text-[12px] lg:text-[13px] font-bold uppercase tracking-widest mb-1">{stat.label}</p>
                        <p className="text-[#1d1d1f] text-[14px] sm:text-[15px] lg:text-[17px] font-bold truncate">{stat.value}</p>
                    </div>
                ))}
            </div>

            <section className="bg-white rounded-[32px] sm:rounded-[40px] p-6 sm:p-8 lg:p-10 shadow-sm border border-[#d2d2d7]/50">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 mb-6 sm:mb-8">
                    <h3 className="text-[24px] sm:text-[26px] lg:text-[22px] font-bold text-[#1d1d1f]">Fast Actions</h3>
                    <div className="px-3 sm:px-4 py-1 sm:py-1.5 bg-[#f5f5f7] rounded-full text-[11px] sm:text-[12px] font-bold text-[#86868b] whitespace-nowrap">UPDATED JUST NOW</div>
                </div>
