    ...
    return (
        <LifeSyncHubPageShell>
            <div className="flex min-h-[min(75vh,calc(100dvh-6rem))] items-center justify-center py-10 sm:py-14">
                <MotionDiv
                    className="flex w-full max-w-sm flex-col items-center gap-4 rounded-[24px] border border-[var(--color-border-strong)]/90 bg-[var(--color-surface)]/95 px-10 py-10 shadow-[0_12px_40px_-14px_rgba(100,90,130,0.12)] ring-1 ring-[var(--mx-color-e8e4ef)]/70"
                    initial={{ opacity: 0, y: 16, scale: 0.98 }}
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    transition={lifeSyncPageTransition}
                >
                    <div className={`flex h-14 w-14 items-center justify-center rounded-2xl shadow-md ring-2 ring-[var(--color-border-strong)] ${isSuccess ? 'bg-gradient-to-br from-[var(--mx-color-c6ff00)] to-[var(--mx-color-9fe870)]' : 'bg-gradient-to-br from-[var(--mx-color-fce7f3)] to-[var(--mx-color-ddd6fe)]'}`}>
                        {isSuccess ? (
                            <svg className="h-7 w-7 text-[var(--mx-color-1a1628)]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                            </svg>
                        ) : (
                            <svg className="h-7 w-7 text-[var(--mx-color-9d174d)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div className="w-full text-center">
                        <p className="text-[15px] font-bold text-[var(--mx-color-1a1628)]">LifeSync OAuth</p>
                        <p className={`mt-1 text-[12px] ${isSuccess ? 'text-emerald-600' : 'text-[var(--mx-color-5b5670)]'}`}>{status}</p>
                    ...
                    <div className="flex gap-1">
                        <span className="lifesync-dot-bounce h-2 w-2 rounded-full bg-[var(--mx-color-c6ff00)]" aria-hidden />
                        <span
                            className="lifesync-dot-bounce lifesync-dot-bounce-delay-1 h-2 w-2 rounded-full bg-[var(--mx-color-a78bfa)]"
                            aria-hidden
                        />
                        <span
                            className="lifesync-dot-bounce lifesync-dot-bounce-delay-2 h-2 w-2 rounded-full bg-[var(--mx-color-5eead4)]"
                            aria-hidden
                        />
                    </div>
                    <p className="text-[11px] text-[var(--mx-color-7c7794)]">Redirecting...</p>
                </MotionDiv>
            </div>
        </LifeSyncHubPageShell>
    )
