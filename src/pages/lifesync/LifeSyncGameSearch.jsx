        ...
        <LifeSyncHubPageShell>
            <div className="space-y-6">
                <header className="lifesync-games-glass rounded-[20px] border border-apple-border/60 bg-white/95 p-5 shadow-sm">
                    <p className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#6e6e73]">LifeSync • GameSearch</p>
                ...
                <section className="lifesync-games-glass rounded-[20px] border border-apple-border/60 bg-white/95 p-5 shadow-sm">
                    <form onSubmit={onSubmit} className="grid gap-3 md:grid-cols-12 md:items-end">
                        <label className="flex flex-col gap-1 md:col-span-5">
                            <span className="text-[12px] font-semibold text-[#515154]">Game name</span>
                            <input
                                type="text"
                                value={queryInput}
                                onChange={(e) => setQueryInput(e.target.value)}
                                placeholder="Elden Ring"
                                className="h-11 rounded-xl border border-apple-border px-3 text-[14px] text-apple-text outline-none ring-[#0071e3]/30 transition focus:ring-2"
                            />
                        </label>
                        <label className="flex flex-col gap-1 md:col-span-2">
                            <span className="text-[12px] font-semibold text-[#515154]">Mode</span>
                            <select
                                value={mode}
                        ...
                        <label className="flex flex-col gap-1 md:col-span-2">
                            <span className="text-[12px] font-semibold text-[#515154]">Max links</span>
                            <input
                                type="number"
                                min="1"
                                max="50"
                        ...
                        <div className="md:col-span-3">
                            <button
                                type="submit"
                                disabled={loading}
                                className="h-11 w-full rounded-xl bg-primary px-4 text-[13px] font-semibold text-[#1a1628] shadow-sm ring-1 ring-[#1a1628]/10 transition hover:brightness-95 disabled:opacity-60"
                            >
                                {loading ? 'Searching...' : 'Search'}
                            </button>
                        </div>
                        <label className="flex flex-col gap-1 md:col-span-12">
                            <span className="text-[12px] font-semibold text-[#515154]">Sources (optional, comma-separated)</span>
                            <input
                                type="text"
                                value={sourcesInput}
                ...
                {query && !loading && !error && (
                    <section className="grid gap-4 lg:grid-cols-12">
                        <article className="lifesync-games-glass rounded-[20px] border border-apple-border/60 bg-white p-5 shadow-sm lg:col-span-5">
                            ...
                            {details?.heroImage && (
                                <div className="mt-3 overflow-hidden rounded-xl border border-[#e5e5ea] bg-apple-bg">
                                    <img
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-2">
                                ...
                                {game?.steam_appid && (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="rounded-full border border-apple-border bg-white px-3 py-1 text-[11px] font-semibold text-apple-text transition hover:border-[#0071e3]"
                                    >
                            ...
                            <div className="mt-3">
                                ...
                                <button
                                    ...
                                    className="ml-2 inline-flex items-center rounded-lg border border-apple-border bg-white px-3 py-1.5 text-[12px] font-semibold text-apple-text transition hover:border-[#0071e3] disabled:opacity-60"
                                >
                        ...
                        <article className="lifesync-games-glass rounded-[20px] border border-apple-border/60 bg-white p-5 shadow-sm lg:col-span-7">
                            ...
                            {links.length > 0 && (
                                <p className="mt-2 text-[11px] text-[#6e6e73]">GameStatus links are hidden. Direct store/provider links only.</p>
                            )}
                            {directStoreLinks.length === 0 ? (
                                <p className="mt-4 text-[13px] text-[#6e6e73]">No direct store links returned for this query.</p>
                            ) : (
                                <div className="mt-4 space-y-3">
                                    {directStoreLinks.map((item, idx) => (
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="lifesync-games-glass block rounded-xl border border-[#e5e5ea] bg-[#fbfbfd] p-3 transition hover:border-[#0071e3]/50 hover:bg-white"
                                        >
                                            <div className="flex flex-wrap items-center justify-between gap-2">
                                                <p className="text-[13px] font-semibold text-apple-text">{item.name || item.title || 'Source link'}</p>
                                                <span className="rounded-full bg-[#eef2ff] px-2 py-1 text-[11px] font-semibold uppercase text-[#4f46e5]">
                                                    {item.source || 'provider'}
                                                </span>
                                            </div>
                                            <p className="mt-1 truncate text-[12px] text-[#6e6e73]">{item.url}</p>
                                        </a>
                                    ))}
                                </div>
                            )}
