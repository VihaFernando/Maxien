...
const DEPARTMENT_COLORS = ["var(--mx-color-0ea5e9)", "var(--mx-color-22c55e)", "var(--mx-color-f59e0b)", "var(--mx-color-ef4444)", "var(--mx-color-8b5cf6)", "var(--mx-color-14b8a6)"]
...
export default function WorkplaceDepartments({
    ...
    const handleSubmit = async (e) => {
        ...
        const basePayload = {
            ...
            color: newDepartment.color || "var(--mx-color-0ea5e9)",
        ...
        <div className="animate-in fade-in duration-500">
            <div className="bg-[var(--color-surface)] rounded-[22px] border border-[var(--mx-color-d2d2d7)]/50 shadow-sm p-5 sm:p-6">
                ...
                {loading ? (
                    ...
                    <div className="py-12 text-center text-[var(--mx-color-86868b)] text-[13px]">No departments yet.</div>
                ) : (
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-6">
                        {filteredDepartments.map((department) => {
                            ...
                            return (
                                <div key={department.id} className="rounded-2xl border border-[var(--mx-color-d2d2d7)]/50 bg-[var(--color-surface)] p-4 sm:p-5 hover:shadow-sm transition-all">
                                    <div className="flex items-start justify-between gap-3 mb-3">
                                        <div className="min-w-0 flex-1">
                                            <p className="text-[15px] font-bold text-[var(--mx-color-1d1d1f)] truncate">{department.name}</p>
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                ...
                                                <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-[var(--mx-color-f5f5f7)] text-[var(--mx-color-1d1d1f)] inline-flex items-center gap-1.5">
                                    ...
                                    {department.description && (
                                        <p className="text-[13px] text-[var(--mx-color-5f6368)] leading-relaxed mb-3">{department.description}</p>
                                    )}

                                    <div className="rounded-xl border border-[var(--mx-color-eef0f3)] bg-[var(--mx-color-fafbfc)] p-3">
                                        <p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--mx-color-86868b)] mb-2">Members</p>
                                        {memberNames.length === 0 ? (
                                            <p className="text-[12px] text-[var(--mx-color-86868b)]">No users assigned</p>
                                        ) : (
                                            <p className="text-[12px] text-[var(--mx-color-374151)] leading-relaxed">{memberNames.join(", ")}</p>
                                        )}
                                    </div>
                                </div>
                            )
                ...
                {showForm && (
                    <div className="pt-6 border-t border-[var(--mx-color-f0f0f0)]">
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="text-[11px] font-bold text-[var(--mx-color-86868b)] uppercase tracking-wider mb-2 block">Department name *</label>
                                <input
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {DEPARTMENT_COLORS.map((c) => (
                                            <button
                                                ...
                                                className={`w-8 h-8 rounded-full border-2 transition-transform ${newDepartment.color === c ? "scale-110 border-[var(--mx-color-1d1d1f)]" : "border-[var(--color-border-strong)]"}`}
                                ...
                                <div>
                                    <label className="text-[11px] font-bold text-[var(--mx-color-86868b)] uppercase tracking-wider mb-2 block">Assign Members</label>
                                    <div className="max-h-40 overflow-y-auto rounded-xl border border-[var(--mx-color-d2d2d7)]/50 bg-[var(--color-surface)] p-2 space-y-1">
                                        {acceptedMembers.length === 0 ? (
                                            <p className="text-[12px] text-[var(--mx-color-86868b)] p-2">No accepted workplace members.</p>
                                        ) : (
                                            acceptedMembers.map((member) => (
                                                <label
                                                    key={member.user_id}
                                                    className="flex items-center gap-2 cursor-pointer hover:bg-[var(--mx-color-f5f5f7)] p-2 rounded-lg"
                                                >
                                                    <input
                                                        type="checkbox"
                                                    ...
                                                    />
                                                    <span className="text-[13px] text-[var(--mx-color-1d1d1f)]">
                                    ...
                                    {selectedMemberLabels.length > 0 && (
                                        <p className="text-[12px] text-[var(--mx-color-86868b)] mt-2">Selected: {selectedMemberLabels.join(", ")}</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button
                                    ...
                                    className="flex-1 py-3 rounded-xl bg-[var(--mx-color-c6ff00)] hover:bg-[var(--mx-color-b8f000)] disabled:opacity-60 text-[var(--mx-color-1d1d1f)] font-bold text-[14px] inline-flex items-center justify-center gap-2"
                                >
