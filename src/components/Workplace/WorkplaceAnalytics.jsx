...
function CompletionBar({ done = 0, total = 0, color = "var(--mx-color-22c55e)" }) {
  const percent = total > 0 ? Math.round((done / total) * 100) : 0
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between text-[11px] text-[var(--mx-color-5f6b7c)]">
        <span>{done}/{total} done</span>
        <span>{percent}%</span>
      </div>
      <div className="h-2.5 rounded-full bg-[var(--mx-color-edf1f5)]">
        <div
  ...
  const departmentPerformance = useMemo(() => {
    ...
    return (departments || []).map((department) => {
      ...
      return {
        ...
        color: department.color || "var(--mx-color-0ea5e9)",
        total: linkedTasks.length,
        done,
      }
    })
    ...
    <div className="animate-in fade-in duration-500">
      <div className="mb-6 rounded-[26px] border border-[var(--mx-color-e8e8ed)] bg-[radial-gradient(120%_120%_at_100%_0%,var(--mx-color-eef4ff)_0%,var(--mx-color-ffffff)_45%,var(--mx-color-ffffff)_100%)] p-5 sm:p-6 shadow-[0_14px_36px_rgba(21, 20, 24,0.05)]">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          {isAdmin && (
            <div className="inline-flex items-center gap-1 rounded-xl bg-[var(--color-surface)]/90 p-1 border border-[var(--mx-color-e5e7eb)] shadow-sm">
              <button
                type="button"
                onClick={() => setScope("company")}
                className={`px-3.5 py-2 text-[12px] font-semibold rounded-lg transition-colors ${scope === "company" ? "bg-[var(--mx-color-111827)] text-white shadow-sm" : "text-[var(--mx-color-6b7280)] hover:text-[var(--mx-color-111827)]"}`}
              >
                Company
              </button>
              <button
                type="button"
                onClick={() => setScope("me")}
                className={`px-3.5 py-2 text-[12px] font-semibold rounded-lg transition-colors ${scope === "me" ? "bg-[var(--mx-color-111827)] text-white shadow-sm" : "text-[var(--mx-color-6b7280)] hover:text-[var(--mx-color-111827)]"}`}
              >
                My Tasks
              </button>
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3.5 mb-6">
        ...
        <StatCard label="Completed" value={baseSummary.done} accent="var(--mx-color-16a34a)" />
        <StatCard label="In Progress" value={baseSummary.inProgress} accent="var(--mx-color-3b82f6)" />
        <StatCard label="To Do" value={baseSummary.todo} accent="var(--mx-color-64748b)" />
        <StatCard label="Overdue" value={baseSummary.overdue} accent="var(--mx-color-ef4444)" />
      </div>

      <div className="rounded-2xl border border-[var(--mx-color-e8e8ed)] bg-[var(--color-surface)] p-5 mb-6 shadow-[0_8px_24px_rgba(21, 20, 24,0.04)]">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[13px] font-bold text-[var(--mx-color-1d1d1f)] inline-flex items-center gap-2.5">
            <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-[var(--mx-color-eef2ff)] text-[var(--mx-color-4f46e5)]">
              <FaChartLine className="w-3.5 h-3.5" />
            </span>
            Completion Rate
          </p>
          <span className="text-[12px] font-semibold text-[var(--mx-color-334155)] rounded-full bg-[var(--mx-color-f1f5f9)] px-2.5 py-1">{baseSummary.completionRate}%</span>
        </div>
        <CompletionBar done={baseSummary.done} total={baseSummary.total} color="var(--mx-color-22c55e)" />
      </div>

      {loading ? (
        <div className="rounded-2xl border border-[var(--mx-color-e8e8ed)] bg-[var(--color-surface)] p-10 text-center text-[13px] text-[var(--mx-color-7a8392)] shadow-[0_8px_24px_rgba(21, 20, 24,0.04)]">Loading analytics...</div>
      ) : (
        <>
          {isAdmin && scope === "company" && (
            <>
              <div className="rounded-2xl border border-[var(--mx-color-e8e8ed)] bg-[var(--color-surface)] p-5 mb-6 shadow-[0_8px_24px_rgba(21, 20, 24,0.04)]">
                <h3 className="text-[14px] font-bold text-[var(--mx-color-1d1d1f)] mb-4 inline-flex items-center gap-2.5">
                  <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-[var(--mx-color-ecfeff)] text-[var(--mx-color-0f766e)]">
                    <FaUsers className="w-3.5 h-3.5" />
                  </span>
                  User Performance
                </h3>
                {userPerformance.length === 0 ? (
                  <p className="text-[12px] text-[var(--mx-color-86868b)]">No active members found.</p>
                ) : (
                  <div className="space-y-3">
                    {userPerformance.map((row) => (
                      <div key={row.memberId} className="rounded-xl border border-[var(--mx-color-ebedf2)] bg-gradient-to-br from-[var(--mx-color-ffffff)] to-[var(--mx-color-f8fafc)] p-3.5">
                        ...
                        <div className="grid grid-cols-3 gap-2 text-[11px] text-[var(--mx-color-64748b)] mb-2">
                          <p>Assigned: <span className="font-semibold text-[var(--mx-color-334155)]">{row.total}</span></p>
                          <p>Done: <span className="font-semibold text-[var(--mx-color-166534)]">{row.done}</span></p>
                          <p>Overdue: <span className="font-semibold text-[var(--mx-color-dc2626)]">{row.overdue}</span></p>
                        </div>
                        <CompletionBar done={row.done} total={row.total} color="var(--mx-color-22c55e)" />
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
                <div className="rounded-2xl border border-[var(--mx-color-e8e8ed)] bg-[var(--color-surface)] p-5 shadow-[0_8px_24px_rgba(21, 20, 24,0.04)]">
                  <h3 className="text-[14px] font-bold text-[var(--mx-color-1d1d1f)] mb-4 inline-flex items-center gap-2.5">
                    <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-[var(--mx-color-eff6ff)] text-[var(--mx-color-1d4ed8)]">
                      <FaFolderOpen className="w-3.5 h-3.5" />
                    </span>
                    Project Completion
                  </h3>
                  {projectPerformance.length === 0 ? (
                    <p className="text-[12px] text-[var(--mx-color-86868b)]">No workplace projects found.</p>
                  ) : (
                    <div className="space-y-3">
                ...
                <div className="rounded-2xl border border-[var(--mx-color-e8e8ed)] bg-[var(--color-surface)] p-5 shadow-[0_8px_24px_rgba(21, 20, 24,0.04)]">
                  <h3 className="text-[14px] font-bold text-[var(--mx-color-1d1d1f)] mb-4 inline-flex items-center gap-2.5">
                    <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-[var(--mx-color-f0fdf4)] text-[var(--mx-color-15803d)]">
                      <FaBuilding className="w-3.5 h-3.5" />
                    </span>
                    Department Completion
                  </h3>
                  {departmentPerformance.length === 0 ? (
                    <p className="text-[12px] text-[var(--mx-color-86868b)]">No departments found.</p>
                  ) : (
                    <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-[var(--mx-color-e8e8ed)] bg-[var(--color-surface)] p-5 shadow-[0_8px_24px_rgba(21, 20, 24,0.04)]">
            <h3 className="text-[14px] font-bold text-[var(--mx-color-1d1d1f)] mb-4 inline-flex items-center gap-2.5">
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-[var(--mx-color-fff7ed)] text-[var(--mx-color-c2410c)]">
                <FaClock className="w-3.5 h-3.5" />
              </span>
              Upcoming Due Tasks
            </h3>
            {upcomingTasks.length === 0 ? (
              <p className="text-[12px] text-[var(--mx-color-86868b)]">No upcoming due tasks in this view.</p>
            ) : (
