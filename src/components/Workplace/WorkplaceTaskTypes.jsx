...
export default function WorkplaceTaskTypes({
    ...
    const resetForm = () => {
        setNewType({ name: "", description: "", color: "#C6FF00", status: "Active" })
        setEditingId(null)
        setShowForm(false)
    }
        ...
        <div className="animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-[22px] border border-[#d2d2d7]/50 shadow-sm p-5 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
                    <div>
                        <h2 className="text-[18px] sm:text-[20px] font-bold text-[#1d1d1f]">Workplace Task Types</h2>
                        <p className="text-[12px] text-[#86868b] mt-1">Manage categories used across workplace tasks and projects.</p>
                    </div>
                    <button
                        onClick={() => {
                            setEditingId(null)
                ...
                {loading ? (
                    ...
                    <div className="py-12 text-center text-[#86868b] text-[13px]">No task types found.</div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        {filteredTypes.map((t) => (
                            <div key={t.id} className="rounded-2xl border border-[#d2d2d7]/50 bg-white p-4 sm:p-5 hover:shadow-sm transition-all">
                                <div className="flex items-start justify-between gap-3 mb-3">
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            />
                                            <p className="text-[15px] font-bold text-[#1d1d1f] truncate">{t.name}</p>
                                        </div>
                                ...
                                {t.description && (
                                    <p className="text-[13px] text-[#5f6368] leading-relaxed mb-2">{t.description}</p>
                                )}
                ...
                {showForm && (
                    <div className="pt-6 border-t border-[#f0f0f0]">
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="text-[11px] font-bold text-[#86868b] uppercase tracking-wider mb-2 block">Type name *</label>
                                <input
                                    value={newType.name}
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <input
                                            ...
                                            className="w-16 h-12 rounded-lg border border-[#d2d2d7] cursor-pointer"
                                        />
                                        <input
                                            value={newType.color}
                                            onChange={(e) => setNewType((v) => ({ ...v, color: e.target.value }))}
                                            className="flex-1 px-4 py-3 bg-[#f5f5f7] rounded-xl border border-transparent focus:border-[#C6FF00]/60 focus:bg-white outline-none text-[14px]"
                                        />
                                    </div>
                                </div>
                                <div>
