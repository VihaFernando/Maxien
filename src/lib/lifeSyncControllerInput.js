...
function keyboardMetaForKey(rawKeyName) {
  ...
  if (keyName === "k" || keyName === "K")
    return { key: String(keyName).toLowerCase(), code: "KeyK", keyCode: 75 };
  if (keyName === "j" || keyName === "J")
    return { key: String(keyName).toLowerCase(), code: "KeyJ", keyCode: 74 };
  if (keyName === "l" || keyName === "L")
    return { key: String(keyName).toLowerCase(), code: "KeyL", keyCode: 76 };
  if (keyName === "f" || keyName === "F")
    return { key: String(keyName).toLowerCase(), code: "KeyF", keyCode: 70 };
...
function patchLegacyKeyboardFields(event, meta) {
  ...
  const charCode =
    typeof meta.key === "string" && meta.key.length === 1
      ? meta.key.charCodeAt(0)
      : 0;
  const entries = [
...
export function dispatchBestEffortIframeMediaKey(iframeEl, key) {
  ...
  if (fireKeyEventTarget(document?.fullscreenElement, keyName))
    dispatched = true;
